                        ...
                        list.map(item =>
                            <div key={'liked-villa-'+item.id} className="mini-cart-item clearfix">
                                <div className="mini-cart-img go-top">
                                    <Link to="/product-details"><img src={process.env.REACT_APP_API_ENDPOINT+"/VillaFE/GetVillaImage?id="+item.imageId}
                                                                     alt="Image"/></Link>
                                    <span onClick={() => unlike(item.id)} className="mini-cart-item-delete"><i className="icon-cancel"/></span>
                                </div>
                                <div className="mini-cart-info go-top">
                                    <h6><Link to="/product-details">{item.ad}</Link></h6>
                                    <span className="mini-cart-quantity"><CurrencyFormat value={item?.fiyat} displayType={'text'} thousandSeparator={true} prefix={currencySymbol(item?.paraBirimi)} /></span>
                                </div>
                            </div>
                        )
                    }
                ...
                <div className="footer-newsletter">
                    <form style={{height:'65px'}}>
                        ...
                        <div className="btn-wrapper">
                            <button onClick={() => {navigator.clipboard.writeText(collectionGuid).then()}} className="theme-btn-1 btn" type="button"><i className="fa-solid fa-copy" /></button>
                        </div>
                    </form>
                </div>:null}
            </div>
